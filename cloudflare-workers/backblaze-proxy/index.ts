...
async function getDemo(zip: Zip, request: Request, demo: string, env: any) {
  ...
  try {
    ...
    const zipEntry = new ZipDeflate(fileName, { level: 6 });

    zip.add(zipEntry);

    const response = await fetch(await new AwsClient({
...
export default {
  async fetch(request: Request, env: {
    ...
    Promise.all(fileList.map(file => getDemo(zip, request, file, env)))
      ...
      .catch(error => {
        if (error instanceof Error) {
          console.error(`error processing demos: ${error.message}`, error.stack);
        } else {
